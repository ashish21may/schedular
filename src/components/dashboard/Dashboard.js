  ...
  return (
    <div className="dashboard">
      ...
      <div className="calender-container">
        <div className="calender-days-name">
          {daysInWeek.map((weekDay) => (
            <div className="calender-cell">{weekDay}</div>
          ))}
        </div>
        {daysInMonth.map((day) => {
          return (
            <div className="calender-week">
              ...
              {day.map((cell) => (
                <div
                  key={cell}
                  id={cell}
                  className={`calender-cell ${cell === selectedCell ? 'colored-cell' : ''}`}
                  onClick={(e)=>meetingDetailsPopUp(e, cell)}
                  onDoubleClick={(e)=>meetingSchedular(e, cell)}
                >
                  <div>{cell}</div>	
                  { scheduledDates.find(value=> Number(value) === Number(cell)) ?  <div className="calender-cell-blue-circle">  </div> : null }
                </div>
              ))}{" "}
            </div>
          );
      ...
      <div className="glassbox">
        {showPopUp ? <PopUp onToggle={closePopUp} selectedCell={selectedCell}/> : null}
      </div>
